...
public final class Numeric
{
   ...
   */
  public static byte accumulate(byte[] array, int first, int last,
				  byte init)
    {
      byte acc = init;
      while (first < last)
   ...
   */
  public static byte inner_product(byte[] array1, byte[] array2,
   ...
   */
  public static int partial_sum(byte[] source, byte[] dest,
   ...
   *                  <code>to + (last - first)</code>.
   */
  public static int adjacent_difference(byte[] source, byte[] dest,
					int first, int last, int to)
    {
      if (first < last) {
	dest[to] = source[first];

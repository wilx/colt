...
public final class Numeric
{
   ...
   */
  public static char accumulate(char[] array, int first, int last,
				  char init)
    {
      char acc = init;
      while (first < last)
   ...
   */
  public static char inner_product(char[] array1, char[] array2,
   ...
   */
  public static int partial_sum(char[] source, char[] dest,
   ...
   *                  <code>to + (last - first)</code>.
   */
  public static int adjacent_difference(char[] source, char[] dest,
					int first, int last, int to)
    {
      if (first < last) {
	dest[to] = source[first];

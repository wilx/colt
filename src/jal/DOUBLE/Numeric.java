...
public final class Numeric
{
   ...
   */
  public static double accumulate(double[] array, int first, int last,
				  double init)
    {
      double acc = init;
      while (first < last)
	acc = (double)(acc + array[first++]);
      return acc;
    }
   ...
   */
  public static double inner_product(double[] array1, double[] array2,
				     int first1, int last1, int first2,
				     double init)
    {
      double acc = init;
      while (first1 < last1)
	acc = (double) (acc + array1[first1++] * array2[first2++]);
      return acc;
    }
   ...
   */
  public static int partial_sum(double[] source, double[] dest,
	...
	while (++first < last) {
	  value = (double) (value + source[first]);
	  dest[++to] = value;
	}
	return to + 1;
      }
   ...
   *                  <code>to + (last - first)</code>.
   */
  public static int adjacent_difference(double[] source, double[] dest,
					int first, int last, int to)
    {
      if (first < last) {
	dest[to] = source[first];
	...
	while (++first < last) {
	  ...
	  dest[++to] = (double) (cur_value - prev_value);

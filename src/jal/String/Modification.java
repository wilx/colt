 ...
 * the the equals() method in class String by default.  Any different notion 
 * of equality (such as String.equalsIgnoreCase() may be represented as a 
 * BinaryPredicate.
 *
...
public final class Modification
{
   ...
   */
  static public void swap_ranges(String[] array1, String[] array2,
				 int first1, int last1, int first2)
    {
      while (first1 < last1) {
	String tmp = array2[first2];
   ...
   *                    input range.
   */
  public static void transform(String[] source, String[] destination,
			       int first, int last, int to,
			       UnaryOperator f)
    {
      while (first < last)
   ...
   *                    input range.
   */
  public static void transform(String[] source1, String[] source2,
			       String[] destination,
   ...
   */
  public static void replace(String[] array, int first, int last,
			     String old_value, String new_value)
    {
      while (first < last) {
	if (array[first].equals(old_value))
   ...
   */
  public static void replace_if(String[] array, int first, int last,
				Predicate p, String new_value)
    {
      while (first < last) {
   ...
   * @param new_value   Value that old_value will be replaced with.
   */
  public static void replace_copy(String[] source, String[] destination,
				  int first, int last, int to,
				  String old_value, String new_value)
    {
      while (first < last) {
	String tmp = source[first++];
	destination[to++] = (tmp.equals(old_value)) ? new_value : tmp;
      }
    }
   ...
   * @param new_value   Value to be substituted for replaced elements.
   */
  public static void replace_copy_if(String[] source, String[] destination,
				     int first, int last, int to,
				     Predicate p, String new_value)
    {
      while (first < last) {
   ...
   */
  public static void fill(String[] array, int first, int last, 
			  String x)
    {
      while(first < last) 
	array[first++] = x;
   ...
   */
  public static void generate(String[] array, int first, int last,
			      Generator f)
    {
      while(first < last) 
	array[first++] = f.apply();
   ...
   */
  public static int remove_if(String[] array, int first, int last, 
			      String x)
    {
      int oldLast = last;
      --first;
      while (true) {
	while (++first < last && !array[first].equals(x)); 
	while (first < --last && array[last].equals(x)); 
	if (first >= last) {
	  return first;
	}
   ...
   *                 are contained in the range <code>[first, i)</code>.
   */
  public static int stable_remove(String[] array, int first, int last, 
				  String x)
    {
   ...
   *                 are contained in the range <code>[first, i)</code>.
   */
  public static int stable_remove_if(String[] array, int first, int last, 
				     Predicate p)
    {
   ...
   *                    is <code>[to, i)</code>.
   */
  static public int remove_copy(String[] source, String[] destination,
				int first, int last, int to,
				String value)
    {
      while (first < last) {
   ...
   *                    is <code>[to, i)</code>.
   */
  static public int remove_copy_if(String[] source, String[] destination,
				   int first, int last, int to,
				   Predicate p)
    {
      while (first < last) {
   ...
   */
  public static int unique(String[] array, int first, int last)
    {
   ...
   *                    is <code>[to, i)</code>.
   */
  public static int unique_copy(String[] source, String[] destination,
				int first, int last, int to)
    {
      if (first >= last)
	return to;
	...
	destination[to] = source[first];
      ...
      while (++first < last) {
	if (!destination[to].equals(source[first]))
	  destination[++to] = source[first];
      }
   ...
   *                   is invalid.
   */
  static public void reverse(String[] array, int first, int last)
    {
      while (first < --last) {
  ...
  public static void reverse_copy(String[] array, int first, int last, int to)
    {
      while (last > first)
	array[to++] = array[--last];
    }
   ...
   */
  public static void reverse_copy(String[] source, String[] destination,
				  int first, int last, int to)
    {
      while (last > first)
   ...
   * @param last     One past the end of the range
   */
  public static void rotate(String[] array, int first, int middle, 
				 int last)
    {
   ...
   * @param to       First element of the output range
   */
  public static void rotate_copy(String[] source, String[] destination,
   ...
   */
  public static void random_shuffle(String[] array, int first, int last,
				    Random RNG)
    {
      for (int i = first + 1; i < last; ++i) {
	int randomPlace =  Math.abs(RNG.nextInt()) % 
	  ((i - first) + 1);
	String tmp = array[randomPlace];
   ...
   * @see   Predicate         
   */
  public static int stable_partition(String[] array, int first, 

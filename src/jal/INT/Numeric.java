...
public final class Numeric
{
   ...
   */
  public static int accumulate(int[] array, int first, int last,
				  int init)
    {
      int acc = init;
      while (first < last)
	acc = (int)(acc + array[first++]);
      return acc;
    }
   ...
   */
  public static int inner_product(int[] array1, int[] array2,
				     int first1, int last1, int first2,
				     int init)
    {
      int acc = init;
      while (first1 < last1)
	acc = (int) (acc + array1[first1++] * array2[first2++]);
      return acc;
    }
   ...
   */
  public static int partial_sum(int[] source, int[] dest,
   ...
   *                  <code>to + (last - first)</code>.
   */
  public static int adjacent_difference(int[] source, int[] dest,
					int first, int last, int to)
    {
      if (first < last) {
	dest[to] = source[first];

	int prev_value = source[first];
	while (++first < last) {
	  int cur_value = source[first];
	  dest[++to] = (int) (cur_value - prev_value);

package jal.String;
import jal.String.VoidFunction;
...
final public class Inspection     
{
   ...
   */
  public static void for_each(String[] array, int first, int last,
			      VoidFunction f)
    {
      while(first < last)
	f.apply(array[first++]);
   ...
   *                 <code>array[i]</code> equals <code>array[i+1]</code>.  Returns
   *                 <code>last</code> if no such iterator exists.
   */
  public static int adjacent_find(String[] array, int first, int last)
    {
      int next = first;
      while (++next < last) {
	if (array[first].equals(array[next]))
	  return first;
	else
	  first = next;
      }
   ...
   *                 <code>E</code> equals <code>x</code>, or <code>last</code> if no such
   *                 element exists.
   */
  public static int find(String[] array, int first, int last, String x)
    {
      while (first < last && !x.equals(array[first]))
	++first;
      return first;
    }
   ...
   * @param x        Value to be searched for.
   * @return         Index of first element <code>E</code> such that
   *                 <code>E</code> does not equal <code>x</code>, or <code>last</code> if no such
   *                 element exists.
   */
  public static int find_not(String[] array, int first, int last, String x)
    {
      while (first < last && x.equals(array[first]))
	++first;
      return first;
   ...
   *                 element exists.
   */
  public static int find_if(String[] array, int first, int last, 
			    Predicate p)
    {
   ...
   */
  public static int find_if_not(String[] array, int first, int last, 
			      Predicate p)
    {
      while (first < last && p.apply(array[first]))
	++first;
   ...
   */
  public static int count_if(String[] array, int first, int last, 
			    Predicate p)
    {
      int counter = 0;
      while (first < last) 
   ...
   */
  public static int count_if_not(String[] array, int first, int last, 
				 Predicate p)
    {
      int counter = 0;
      while (first < last) 
   ...
   *                 <code>array1[i]</code> does not equal <code>array2[first2 + (i-first1)]</code>,
   *                 or <code>last1</code> if no such index in the range
   *                 <code>[first1,last1)</code> exists.
   */
  public static int mismatch(String[] array1, String[] array2,
			     int first1, int last1, int first2)
    {
      while(first1 < last1 && array1[first1].equals(array2[first2])) {
	++first1;
	++first2;
      }
      return first1;
   ...
   *                 <code>array1[i]</code> equals <code>array2[first2 + (i-first1)]</code>,
   ...
   *                 condition 
   *                 <code>array1[first1+n]</code> equals <code>array2[first1+n]</code>
   *                 is satisfied, or <code>last1</code> if no such
   *                 index exists.
   */
  public static int search(String[] array1, String[] array2,
	...
	return last1;
      ...
      while (cur2 < last2) {
	if (!array1[cur1++].equals(array2[cur2++])) {
	  if (len1 == len2)
	    return last1;
	  else {
	    cur1 = ++first1;

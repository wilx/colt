...
public class Converter
{
	...
	{
		StringBuilder buf = new StringBuilder();
		buf.append("Range: [").append(axis.lowerEdge()).append(",").append(axis.upperEdge()).append(")");
		buf.append(", Bins: ").append(axis.bins());
		buf.append(", Bin edges: ").append(toString(edges(axis))).append("\n");
		return buf.toString();
	}
	/** 
	 * Returns a string representation of the given argument.
	...
	public String toXML(IHistogram1D h)
	{
			...
			buf.append("<bins1d title=\"").append(h.title()).append("\">"); buf.append(sep);
			for (int i=0; i<h.xAxis().bins(); i++)
			{
				buf.append(h.binEntries(i)).append(",").append(h.binError(i)); buf.append(sep);
			}
			...
			buf.append("<statistic name=\"Entries\" value=\"").append(h.entries()).append("\"/>"); buf.append(sep);
			buf.append("<statistic name=\"Underflow\" value=\"").append(h.binEntries(h.UNDERFLOW)).append("\"/>"); buf.append(sep);
			buf.append("<statistic name=\"Overflow\" value=\"").append(h.binEntries(h.OVERFLOW)).append("\"/>"); buf.append(sep);
			if (!Double.isNaN(h.mean())) {
				buf.append("<statistic name=\"Mean\" value=\"").append(h.mean()).append("\"/>"); buf.append(sep);
			}
			if (!Double.isNaN(h.rms())) {
				buf.append("<statistic name=\"RMS\" value=\"").append(h.rms()).append("\"/>"); buf.append(sep);
			}
			buf.append("</statistics>"); buf.append(sep);
			buf.append("</data1d>"); buf.append(sep);
			buf.append("</dataArea>"); buf.append(sep);
	...
	public String toXML(IHistogram2D h)
	{
			...
			out.append("<bins2d title=\"").append(h.title()).append("\" xSize=\"").append(h.xAxis().bins()).append("\" ySize=\"").append(h.yAxis().bins()).append("\">"); out.append(sep);
			for (int i=0; i<h.xAxis().bins(); i++)
				for (int j=0; j<h.yAxis().bins(); j++)
			{
				out.append(h.binEntries(i, j)).append(",").append(h.binError(i, j)); out.append(sep);
			}

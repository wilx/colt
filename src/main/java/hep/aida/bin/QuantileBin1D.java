...
public String compareWith(AbstractBin1D other) {
	...
	if (other instanceof QuantileBin1D) {
		...
		buf.append("25%, 50% and 75% Quantiles: ").append(relError(quantile(0.25), q.quantile(0.25))).append(", ").append(relError(quantile(0.5), q.quantile(0.5))).append(", ").append(relError(quantile(0.75), q.quantile(0.75)));
		buf.append("\nquantileInverse(mean): ").append(relError(quantileInverse(mean()), q.quantileInverse(q.mean()))).append(" %");
		buf.append("\n");
	}
	return buf.toString();
}
...
public synchronized String toString() {
	...
	buf.append("25%, 50%, 75% Quantiles: ").append(quantile(0.25)).append(", ").append(quantile(0.5)).append(", ").append(quantile(0.75));
	//buf.append("10%, 25%, 50%, 75%, 90% Quantiles: "+quantile(0.1) + ", "+ quantile(0.25) + ", "+ quantile(0.5) + ", " + quantile(0.75) + ", " + quantile(0.9));
	buf.append("\nquantileInverse(median): ").append(quantileInverse(median()));
	buf.append("\n");
	return buf.toString();
}
}

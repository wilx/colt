...
public class Statistic {
	 ...
	 */ 
	public static final VectorVectorFunction EUCLID = (a, b) -> Math.sqrt(a.aggregate(b, F.plus, F.chain(F.square,F.minus)));

	/**
	 * Bray-Curtis distance function; <tt>Sum( abs(x[i]-y[i]) )  /  Sum( x[i]+y[i] )</tt>.
	 */ 
	public static final VectorVectorFunction BRAY_CURTIS = (a, b) -> a.aggregate(b, F.plus, F.chain(F.abs,F.minus)) / a.aggregate(b, F.plus, F.plus);
	 ...
	 */ 
	public static final VectorVectorFunction MAXIMUM = (a, b) -> a.aggregate(b, F.max, F.chain(F.abs,F.minus));

	/**
	 * Manhattan distance function; <tt>Sum( abs(x[i]-y[i]) )</tt>.
	 */ 
	public static final VectorVectorFunction MANHATTAN = (a, b) -> a.aggregate(b, F.plus, F.chain(F.abs,F.minus));

...
public String toString() {
	...
	try { buf.append(this.isNonsingular());}
	catch (IllegalArgumentException exc) { buf.append(unknown).append(exc.getMessage()); }
	
	buf.append("\ndet = ");
	try { buf.append(this.det());}
	catch (IllegalArgumentException exc) { buf.append(unknown).append(exc.getMessage()); }
	
	buf.append("\npivot = ");
	try { buf.append(new cern.colt.list.IntArrayList(this.getPivot()));}
	catch (IllegalArgumentException exc) { buf.append(unknown).append(exc.getMessage()); }
	
	buf.append("\n\nL = ");
	try { buf.append(this.getL());}
	catch (IllegalArgumentException exc) { buf.append(unknown).append(exc.getMessage()); }
	
	buf.append("\n\nU = ");
	try { buf.append(this.getU());}
	catch (IllegalArgumentException exc) { buf.append(unknown).append(exc.getMessage()); }
	
	buf.append("\n\ninverse(A) = ");
	DoubleMatrix2D identity = cern.colt.matrix.DoubleFactory2D.dense.identity(LU.rows());
	try { this.solve(identity); buf.append(identity); }
	catch (IllegalArgumentException exc) { buf.append(unknown).append(exc.getMessage()); }
